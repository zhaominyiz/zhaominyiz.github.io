...
var s_account="smesonymusiccojp"
...
s.visitorNamespace="sonymusic"
s.trackingServer="sonymusic.112.2o7.net"

/* Basic Variables Last Modified 2013/5/7 by WSM */
if(!s.server || s.server=="") s.server=window.location.host;

/* TimeParting Plugin Configuration Last Modified 2013/5/7 by WSM */
s.dstStart="1/1/2008";
s.dstEnd="1/1/2008";
...
function s_doPlugins(s) {
	...
	/* getArtistDir v0.1 */
	s.prop1=s.getArtistDir();
	
	/* prop11: getLabelDir v0.1 Last Modified 2013/5/7 by WSM */
	if(!s.prop11 || s.prop11=="") s.prop11=s.getLabelDir();

	/* prop12: getIntDir v0.1 Last Modified 2013/5/7 by WSM */
	if(!s.prop12 || s.prop12=="") s.prop12=s.getIntDir();


	/* s.crossVisitParticipation Example: 1.6 */
	s.prop2 = s.crossVisitParticipation(s.prop1,'s_cvp','90','3','>','');
	
	/* linkHandler */
	s.bannerLink=s.linkHandler("cnt.html");
	if(s.bannerLink){
		s.linkTrackVars="prop3";
		s.prop3=s.getQueryParam("banner","",s.bannerLink);
	}

	/* ?id= count */
	if(s.getQueryParam('id')){
		s.prop4=location.href;
	}
	
	/* prop6: full URL added on 2010-02-22 */
	if(!s.prop6 || s.prop6=="") s.prop6=location.href;

	/* prop5: Title Tag (document.title) Last Modified 2013/5/7 by WSM */
	if(!s.prop5 || s.prop5=="") s.prop5=document.title;

	/* eVar4: new-repeat Last Modified 2013/5/7 by WSM */
	s.eVar4=s.getNewRepeat(90);

	/* prop8,eVar8: TimeParting Last Modified 2013/5/7 by WSM */
...
/*
 * Plugin: getArtistDir v0.1 - Return artists dirname from filepath
 */
s.getArtistDir = new Function(""
+"var v=location.pathname;var arr=v.split('/');var len=arr.length;var"
+" rv='';if(v.indexOf('/Music/Arch')==0&&len>4){rv=arr[4];}else if(v."
+"indexOf('/Music/International/Arch')==0&&len>5){rv=arr[5];}return r"
+"v;");

/*
 * Plugin: getLabelDir v0.1 - Return Label dirname from filepath
 */
s.getLabelDir = new Function(""
+"var v=location.pathname;var arr=v.split('/');var len=arr.length;var"
+" rv='';if(v.indexOf('/Music/Arch')==0&&len>4){rv=arr[3];}else if(v."
+"indexOf('/Music/International/Arch')==0&&len>5){rv=arr[4];}return r"
+"v;");

/*
 * Plugin: getIntDirDir v0.1 - Return Label dirname from filepath
 */
s.getIntDir = new Function(""
+"var v=location.pathname;var arr=v.split('/');var len=arr.length;var"
+" rv='';if(v.indexOf('/Music/Arch')==0&&len>4){rv='Domestic';}else i"
+"f(v.indexOf('/Music/International/Arch')==0&&len>5){rv='Internation"
+"al';}return rv;");
...
/*
 * Plugin: getTimeParting 2.0v1 - Set timeparting values based on time zone
 * v1: Customized format "p"
 */
